...
This code contains the function smiles(molecule), which converts a Molecule object to a SMILES string:
http://en.wikipedia.org/wiki/SMILES
Once a Molecule has been converted to a SMILES string, we can render it as an SVG using serverRender.py.
...
def smiles(molecule):
    ...
    (in the latter case, smiles(mol) is applied for mol in the list)
